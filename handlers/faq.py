...
async def faq_q2(callback: CallbackQuery):
    ...
    text = (
        ...
        "При этом надёжность владения полностью сохраняется — права фиксируются регистратором официально."
    )
    await callback.message.edit_text(text, reply_markup=faq_back_keyboard())
    await callback.answer()
...
async def faq_q3(callback: CallbackQuery):
    update_last_activity(str(callback.from_user.id), "faq")
    text = (
        "Оферта — это дата, когда инвестор может вернуть облигации эмитенту и получить номинал + купон. "
        "Если облигации не предъявляются к оферте, выплаты продолжаются до следующей установленной даты."
    )
    await callback.message.edit_text(text, reply_markup=faq_back_keyboard())
    await callback.answer()
...
async def faq_q4(callback: CallbackQuery):
    update_last_activity(str(callback.from_user.id), "faq")
    text = (
        "АО \"НФК-СИ\" входит в группу дочерних компаний Пионер-Лизинга. "
        "Привлечённые средства направляются в лизинговые проекты группы: "
        "транспорт, оборудование, стартапы и другие направления. "
        "Механика напоминает банковскую: средства привлекаются под одну ставку, "
        "а используются в деятельности под более высокую."
    )
    await callback.message.edit_text(text, reply_markup=faq_back_keyboard())
    await callback.answer()
...
async def faq_q5(callback: CallbackQuery):
    ...
    text = (
        ...
        "За детальной инструкцией лучше обратиться к менеджеру."
    )
    await callback.message.edit_text(text, reply_markup=faq_back_keyboard())
    await callback.answer()
...
async def faq_q7(callback: CallbackQuery):
    update_last_activity(str(callback.from_user.id), "faq")
    text = (
        "Купонный доход облагается НДФЛ 13% (для резидентов РФ). "
...
async def faq_more(callback: CallbackQuery):
    ...
    text = (
        "Если вы только начинаете знакомиться с инвестициями, вам может быть полезно "
        "пройти обучение по ценным бумагам.\n\n"
        "Если вопросы касаются компании или покупки облигаций — лучше обратиться к менеджеру."
    )
